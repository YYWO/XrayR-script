...
install() {
    bash <(curl -Ls https://raw.githubusercontent.com/yowiv/XrayR-script/refs/heads/main/install.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
#        return 0
#    fi
    bash <(curl -Ls https://raw.githubusercontent.com/yowiv/XrayR-script/refs/heads/main/install.sh) $version
    if [[ $? == 0 ]]; then
